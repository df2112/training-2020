...
import DoctorSearch, {validate} from './index'
...
test('DoctorSearch renders without errors', () => {
    const wrapper = shallow(
        <DoctorSearch handleSubmit={() => undefined} onSubmit={() => undefined} />
    )
    expect(wrapper).toHaveLength(1)
    expect(wrapper.hasClass('c-doctor-search')).toBe(true)
})
...
test('Analytics manager tracks email submit errors', () => {
    ...
    const wrapper = shallow(<DoctorSearch />)
...
test('If an onSubmit prop is given, call it on submission of a valid email', () => {
    ...
    const wrapper = shallow(<DoctorSearch onSubmit={onSubmit} />)
    wrapper.instance().setState({value: 'goodemail@mobify.com'})
