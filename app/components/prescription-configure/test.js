...
import PrescriptionConfigure, {validate} from './index'
...
test('PrescriptionConfigure renders without errors', () => {
    const wrapper = shallow(
        <PrescriptionConfigure handleSubmit={() => undefined} onSubmit={() => undefined} />
    )
    expect(wrapper).toHaveLength(1)
    expect(wrapper.hasClass('c-prescription-configure')).toBe(true)
})
...
test('Analytics manager tracks email submit errors', () => {
    ...
    const wrapper = shallow(<PrescriptionConfigure />)
...
test('If an onSubmit prop is given, call it on submission of a valid email', () => {
    ...
    const wrapper = shallow(<PrescriptionConfigure onSubmit={onSubmit} />)
    wrapper.instance().setState({value: 'goodemail@mobify.com'})
