...
import PrescriptionsGrid, {validate} from './index'
...
test('PrescriptionsGrid renders without errors', () => {
    const wrapper = shallow(
        <PrescriptionsGrid handleSubmit={() => undefined} onSubmit={() => undefined} />
    )
...
test('Analytics manager tracks email submit errors', () => {
    ...
    const wrapper = shallow(<PrescriptionsGrid />)
...
test('If an onSubmit prop is given, call it on submission of a valid email', () => {
    ...
    const wrapper = shallow(<PrescriptionsGrid onSubmit={onSubmit} />)
    wrapper.instance().setState({value: 'goodemail@mobify.com'})
