...
import DrugSearch, {validate} from './index'
...
test('DrugSearch renders without errors', () => {
    const wrapper = shallow(
        <DrugSearch handleSubmit={() => undefined} onSubmit={() => undefined} />
    )
    expect(wrapper).toHaveLength(1)
    expect(wrapper.hasClass('c-drug-search')).toBe(true)
})
...
test('Analytics manager tracks email submit errors', () => {
    ...
    const wrapper = shallow(<DrugSearch />)
...
test('If an onSubmit prop is given, call it on submission of a valid email', () => {
    ...
    const wrapper = shallow(<DrugSearch onSubmit={onSubmit} />)
    wrapper.instance().setState({value: 'goodemail@mobify.com'})
