...
import DoctorAddNew, {validate} from './index'
...
test('DoctorAddNew renders without errors', () => {
    const wrapper = shallow(
        <DoctorAddNew handleSubmit={() => undefined} onSubmit={() => undefined} />
    )
    expect(wrapper).toHaveLength(1)
    expect(wrapper.hasClass('c-doctor-add-new')).toBe(true)
})
...
test('Analytics manager tracks email submit errors', () => {
    ...
    const wrapper = shallow(<DoctorAddNew />)
...
test('If an onSubmit prop is given, call it on submission of a valid email', () => {
    ...
    const wrapper = shallow(<DoctorAddNew onSubmit={onSubmit} />)
    wrapper.instance().setState({value: 'goodemail@mobify.com'})
